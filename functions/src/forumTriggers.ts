...
export const onReplyCreated = onDocumentCreated("posts/{postId}/comments/{commentId}", async (event) => {
    ...
    try {
        ...
        if (parentId) {
            ...
            if (parentSnap.exists) {
                ...
                if (parentData && parentData.authorId !== authorId && parentData.authorId !== postAuthorId) {
                     const notifRef = db.collection('users').doc(parentData.authorId).collection('notifications').doc();
                     batch.set(notifRef, {
                        type: 'reply',
